...
namespace Test.Odb.Test.Update.Nullobject
{
    ...
    public class TestNullObject : ODBTest
	{
        ...
        public virtual void Test1()
		{
			...
			odb.Close();
			odb = Open("sict");
			IObjects<AT> ats = odb.GetObjects<AT>();
			int nbAts = ats.Count;
			at = (AT)ats.GetFirst();
			AT newAT = null;
			SensorAT newSensor = null;
			IQuery query = new CriteriaQuery(Where.Equal("at.name", at.GetName()));
			query.OrderByAsc("lane");
			IObjects<SensorAT> sensors = odb.GetObjects<SensorAT>(query);
		...
		public static AT CreateAT(User
			 user)
		{
		...
		public static SensorAT CreateSensor(User
			...
			)
		{
			...
			Profile profileOper = new Profile
				("operador");
		...
		/// <exception cref="System.Exception"></exception>
		public static void CreateFunctionProfile(IOdb odb, Profile
			 admin, Profile oper)
		{
