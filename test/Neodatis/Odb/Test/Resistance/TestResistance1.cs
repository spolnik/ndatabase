...
namespace Resistance
{
    ...
    public class TestResistance1 : ODBTest
    {
        ...
        public virtual void Test1WithCommit()
        {
            ...
            var f1 = new Function("function 1");
            // Create Objects
        ...
        public virtual void Test1WithCommit4()
        {
            ...
            DeleteBase(FileName);
            var odb = Open(FileName);
            var f1 = new Function("function1");
            odb.Store(f1);
            var f2 = new Function("function2");
            odb.Store(f2);
            odb.Close();
            odb = Open(FileName);
            var objects = odb.GetObjects<Function>();
            Function f = null;
            Println("got the object " + objects.Count);
            for (var k = 0; k < 2; k++)
            {
        ...
        public virtual void Test1WithoutCommit()
        {
            ...
            var uncommited =
                engine.GetSession(true).GetMetaModel().GetClassInfo(typeof (User).FullName, true).GetUncommittedZoneInfo
                    ();
            CIZoneInfo commited =
                engine.GetSession(true).GetMetaModel().GetClassInfo(typeof (User).FullName, true).GetCommitedZoneInfo();
