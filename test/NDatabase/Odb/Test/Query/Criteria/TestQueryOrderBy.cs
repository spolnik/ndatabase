...
namespace Test.NDatabase.Odb.Test.Query.Criteria
{
    ...
    public class TestQueryOrderBy : ODBTest
    {
        ...
        public virtual void Test5()
        {
            ...
            odb.Close();
            odb = Open(baseName);
            IQuery q = new CriteriaQuery(typeof(VO.Login.Function), Where.IsNotNull("name"));
            // q.orderByAsc("name");
            var objects = odb.GetObjects<VO.Login.Function>(q, true, 0, 10);
            AssertEquals(size, objects.Count);
            odb.Close();
            odb = Open(baseName);
            q = new CriteriaQuery(typeof(VO.Login.Function), Where.IsNotNull("name"));
            q.OrderByAsc("name");
        ...
        public virtual void Test6()
        {
            ...
            odb.Close();
            odb = Open(baseName);
            IQuery q = new CriteriaQuery(typeof(VO.Login.Function), Where.IsNull("name"));
            // q.orderByAsc("name");
            var objects = odb.GetObjects<VO.Login.Function>(q, true, 0, 10);
            AssertEquals(1, objects.Count);
            odb.Close();
            odb = Open(baseName);
            q = new CriteriaQuery(typeof(VO.Login.Function), Where.IsNull("name"));
            q.OrderByAsc("name");
            objects = odb.GetObjects<VO.Login.Function>(q, true, 0, 10);
            AssertEquals(1, objects.Count);
            odb.Close();
