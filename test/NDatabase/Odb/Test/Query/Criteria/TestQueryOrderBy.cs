...
namespace Test.NDatabase.Odb.Test.Query.Criteria
{
    ...
    public class TestQueryOrderBy : ODBTest
    {
        ...
        public virtual void Test4()
        {
            ...
            // q.orderByAsc("name");
            var objects = odb.Query<VO.Login.Function>(q, true, 0, 2);
            AssertEquals(2, objects.Count);
            odb.Close();
            odb = Open(baseName);
            q = odb.CreateCriteriaQuery<VO.Login.Function>();
            q.OrderByAsc("name");
            objects = odb.Query<VO.Login.Function>(q, true, 0, 2);
            AssertEquals(2, objects.Count);
            odb.Close();
            odb = Open(baseName);
            q = odb.CreateCriteriaQuery<VO.Login.Function>();
